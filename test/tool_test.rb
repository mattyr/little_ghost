...
class ToolTest < Minitest::Test
  ...
  def test_execute_preserves_advanced_result_artifacts
    artifact = LittleGhost::Artifact.new(data: "image", media_type: "image/png")
    ...
    assert_predicate result.artifacts, :frozen?
  end

  def test_result_rejects_non_artifacts
    assert_raises(ArgumentError) do
      LittleGhost::Tool::Result.new(value: "result", artifacts: ["not an artifact"])
    end
  end
